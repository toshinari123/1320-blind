...
def read(s):
    ...
    name = 'tmp/' + datetime.now().strftime("%d%m%Y%H%M%S") + '.mp3'
    audio.save(name)
    playsound(name)
    os.remove(name)
...
def localize_objects(path):
    ...
    detector.loadModel()
    return detector.detectObjectsFromImage(input_image=path, output_image_path=path[:-4] + '_imageai.png')
...
while True:
    ...
    name = 'out/' + datetime.now().strftime("%d%m%Y%H%M%S") + '.png'
    ss.save(name)
    desc = gen_desc(name)
    print(desc)
    read(desc)
